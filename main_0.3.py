...
def start(message):
    ...
    bot.send_message(message.chat.id, text="""📈Приветствую, инвестор!

Сегодня для Вас откроются несколько тайн финансов и инвестиций, которые помогут приумножить капитал💸
...
Скорее жмите на кнопку ниже, чтобы приблизить финансовую свободу👇""",
...
def handle_callback(call):
    ...
    if call.data == 'решение 1':
        ...
        bot.send_message(call.message.chat.id, text=f"""💰Урок 1. Первые шаги будущего миллионера. Куда инвестировать?
...
После просмотра скорее переходите ко второму уроку, где будут конкретные стратегии инвестиций👇""", reply_markup=keyboard, parse_mode="HTML")
        ...
        bot.send_message(call.message.chat.id, text=f"""💵Урок 2. Прибыльные стратегии на 20%+ годовых 

В этом уроке точный алгоритм действий, как с минимальным рисками получать в 3-4 раза больше, чем на банковском депозите.
...
В следующем уроке вы узнаете о том, как составить портфель, как НЕ прогореть и как инвестировать в криптовалюты. 

Смотрите его по ссылке ниже👇""", reply_markup=keyboard, parse_mode="HTML")
        ...
        bot.send_message(call.message.chat.id, text="""💲Урок 3. Ответы на вопросы. 
...
Приятного просмотра🥳""",
        ...
        bot.send_message(call.message.chat.id, text=f"""Ура! Вы сделали первый шаг к финансовой свободе!

📲Остаемся на связи:

▶️<a href="{"https://t.me/alenakladko"}">{"Телеграмм-канал с новостями"}</a>◀️

▶️<a href="{"https://instagram.com/alena.kladko?igshid=NTc4MTIwNjQ2YQ=="}">{"Инстаграмм с полезными видео"}</a>◀️
